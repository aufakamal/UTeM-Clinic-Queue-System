<?php
    ...
    $upcomingSQL = "SELECT a.appointmentID, a.appointmentType, a.appointmentStatus, a.appointmentFor, a.dependantName, a.dependantRelationship,
                            ts.slotDate, ts.startTime, ts.endTime, ts.slotType
                    FROM appointment a
    ...
    $previousSQL = "SELECT a.appointmentID, a.appointmentType, a.appointmentStatus, a.appointmentFor, a.dependantName, a.dependantRelationship,
                        ts.slotDate, ts.startTime, ts.endTime, ts.slotType
                    FROM appointment a
                    JOIN time_slot ts ON a.slotID = ts.slotID
                    WHERE a.userID = '$userID' AND a.appointmentStatus IN ('Completed', 'Cancelled', 'No Show')
        ...
        <article id="upcomingRecords">
            ...
            <div class="appointmentRecordTable">
                <table>
                    ...
                    <tr>
                        <td><?= date('d/m/Y', strtotime($row['slotDate'])) ?></td>
                        <td><?= $row['slotType'] ?></td>
                        <td><?= date('g:i A', strtotime($row['startTime'])) ?> - <?= date('g:i A', strtotime($row['endTime'])) ?></td>
                        <td>
                            <?php
                        ...
                        <td>
                            <form action="cancelAppointment.php" method="POST" onsubmit="return confirm('Are you sure you want to cancel this appointment?');">
