<footer>
    <h2>Contact Us</h2>

<div class="footerGrid">
    <div class="socialItem">
        <a href="https://www.instagram.com/pusatkesihatanutem/?hl=en">
            <img class="icon" src="patientImages/instagramIcon.png" alt="Instagram Icon">
            <span>Instagram</span>
        </a>
    </div>

    <div class="contactItem">
        <img class="icon" src="patientImages/phoneIcon.png" alt="Phone Icon">
        <span>06-2292308</span>
    </div>

    <div class="socialItem">
        <a href="https://www.facebook.com/PusatKesihatanUTeM/">
            <img class="icon" src="patientImages/facebookIcon.png" alt="Facebook Icon">
            <span>Facebook</span>
        </a>
    </div>

    <div class="contactItem">
        <img class="icon" src="patientImages/emailIcon.png" alt="Email Icon">
        <a href="mailto:pusatkesihatanutem@utem.edu.my">
            pusatkesihatanutem@utem.edu.my
        </a>
    </div>

    <div class="socialItem">
        <a href="https://www.tiktok.com/@utempku">
            <img class="icon" src="patientImages/tiktokIcon.png" alt="TikTok Icon">
            <span>TikTok</span>
        </a>
    </div>

    <div class="contactItem">
        <img class="icon" src="patientImages/locationIcon.png" alt="Location Icon">
        <span>
            Universiti Teknikal Malaysia Melaka, Jalan Hang Tuah Jaya,
            76100 Durian Tunggal, Melaka.
        </span>
    </div>
</div>

<p class="copyright">
    Copyright &copy; 2026 PROJECT SYSTEM DEVELOPMENT WORKSHOP S1G2
</p>

</footer>
