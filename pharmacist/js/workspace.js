...
const patientRecordView = document.querySelector("#patientRecordView");
...
const workspaceVisitsHistory = document.querySelector("#workspaceVisitsHistory");
...
function buildPrescriptionTable(queue) {
    const prescriptionListTable = document.querySelector("#prescriptionListTable");
...
viewButtons.forEach((button) => {
    button.addEventListener("click", () => {
        ...
        const workspacePrescriptionID = document.querySelector("#workspacePrescriptionID");

        if (workspacePrescriptionID) {
            workspacePrescriptionID.value = patients[queue].prescriptionID;
        }
...
function searchPatientLive() {
    const searchPatientInput = document.querySelector("#searchPatientInput");
    ...
    searchPatientsData.forEach((patient) => {
        const patientInfo = patient.fullName.toLowerCase() + " " + patient.userID.toLowerCase();
...
function setWorkspaceAction(action) {
    const workspaceAction = document.querySelector("#workspaceAction");
...
function validateWorkspaceForm() {
    const workspaceActionInput = document.querySelector("#workspaceAction");
...
function switchSearchTab(tab, btn) {
    ...
    const overviewSection = document.querySelector("#searchOverviewSection");
    const visitsSection = document.querySelector("#searchVisitsSection");
...
const prescriptionPopup = document.querySelector("#prescriptionPopup");
const closePrescriptionBtn = document.querySelector(".closePrescriptionBtn");
const popupMedicine = document.querySelector("#popupMedicine");
const popupCurrentStock = document.querySelector("#popupCurrentStock");
const popupRemainingStock = document.querySelector("#popupRemainingStock");
const popupQuantity = document.querySelector("#popupQuantity");
const popupDosage = document.querySelector("#popupDosage");
const popupFrequency = document.querySelector("#popupFrequency");
const popupDuration = document.querySelector("#popupDuration");
const popupInstruction = document.querySelector("#popupInstruction");
const popupPrescriptionItemID = document.querySelector("#popupPrescriptionItemID");
const savePopupBtn = document.querySelector("#savePopupBtn");

if (closePrescriptionBtn && prescriptionPopup) {
...
function initialiseEditButtons() {
    document.querySelectorAll(".editPrescriptionBtn").forEach((button) => {
        button.onclick = () => {
            const popupQueueKey = document.querySelector("#popupQueueKey");

            if (popupQueueKey) {
                popupQueueKey.value = selectedQueue;
            }

            if (popupPrescriptionItemID) {
                popupPrescriptionItemID.value = button.dataset.id;
            }

            if (popupMedicine) {
                popupMedicine.value = button.dataset.medicine;
            }

            updateStockInfo(parseInt(button.dataset.stock));

            if (popupQuantity) {
                popupQuantity.value = button.dataset.quantity;
            }

            if (popupDosage) {
                popupDosage.value = button.dataset.dosage;
            }

            if (popupFrequency) {
                popupFrequency.value = button.dataset.frequency;
            }

            if (popupDuration) {
                popupDuration.value = button.dataset.duration;
            }

            if (popupInstruction) {
                popupInstruction.value = button.dataset.instruction;
            }

            if (prescriptionPopup) {
...
if (popupMedicine) {
    popupMedicine.addEventListener("change", () => {
        const selectedMedicine = medicineList.find((medicine) => {
            return medicine.medicineID == popupMedicine.value;
        });

        if (selectedMedicine) {
            updateStockInfo(parseInt(selectedMedicine.stockQuantity));
        }
    });
}

function updateStockInfo(stock) {
    if (!popupCurrentStock || !popupQuantity || !popupRemainingStock) {
        return;
    }
    ...
    popupRemainingStock.textContent = remaining + " units";
}
...
if (typeof reopenQueue !== "undefined" && reopenQueue !== "") {
    const button = document.querySelector(`.viewBtn[data-queue="${reopenQueue}"]`);
...
if (typeof workspaceError !== "undefined" && workspaceError === "notready") {
    alert("Prescription must be marked Ready before dispensing.");
}

/* =========================
   SEARCH PATIENT RECORD VIEW
========================= */
